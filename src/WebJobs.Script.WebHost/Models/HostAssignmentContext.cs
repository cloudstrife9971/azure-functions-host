    ...
    public class HostAssignmentContext
    {
        ...
        public void ApplyAppSettings(IEnvironment environment, ILogger logger)
        {
            ...
            if (EasyAuthSettings != null)
            {
                ...
                {
                    logger.LogWarning($"ApplyAppSettings is adding {EnvironmentSettingNames.EasyAuthEnabled} = {EasyAuthSettings.SiteAuthEnabled.ToString()}");
                    environment.SetEnvironmentVariable(EnvironmentSettingNames.EasyAuthEnabled, EasyAuthSettings.SiteAuthEnabled.ToString());
                }
                else
                {
                    logger.LogWarning($"ApplyAppSettings operating on existing {EnvironmentSettingNames.EasyAuthEnabled} = {environment.GetEnvironmentVariable(EnvironmentSettingNames.EasyAuthEnabled)}");
                }

                logger.LogWarning($"ApplyAppSettings operating on existing {EnvironmentSettingNames.EasyAuthClientId} = {EasyAuthSettings.SiteAuthClientId}");
